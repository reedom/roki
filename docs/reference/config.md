...
| `extension.orchestrator.stall_seconds` | roki-mvp (orchestrator session) | no | Orchestrator-stall window in seconds (default `120`). If the orchestrator emits no stdout for this many seconds the daemon SIGTERMs it and routes the issue to `Inactive(reason=orchestrator_crash)` per Req 5.3 | [19-orchestrator-session](../fr/19-orchestrator-session.md) | roki-mvp Req 2.11, Req 5.3 |

...
| `[linear].endpoint` | no | Linear GraphQL endpoint URL (default `https://api.linear.app/graphql`). Operators may point this at Linear's EU endpoint or a self-hosted GraphQL proxy; tests redirect production bootstrap at a wiremock by setting this slot | Refuses startup if empty or not an `http://` / `https://` URL | [03-linear-integration](../fr/03-linear-integration.md) | roki-mvp Req 2.13, Req 3.4 |

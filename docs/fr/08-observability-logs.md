...
Operators must diagnose daemon behavior, admission decisions, cycle outcomes, and per-state subprocess output from inspecting these three tiers alone. There is no separate debug mode and no separate "per-issue debug log directory": per-visit raw captures are always on disk under the session tempdir; structured events always flow through tracing.
...
- **Artifact contents** (`requirements.md`, `review.md`, etc.): the daemon does not parse artifacts, so it cannot leak them. Per-visit captures may contain artifact references the operator's cli line emitted; the operator owns redaction in that path.

### Surfacing subprocess stderr in the event log

For every state visit, the daemon emits one `state_completed` (or `state_failed`) event when the subprocess exits, with summary fields including a truncated head + tail slice of stderr (size configurable in `roki.toml [log]`; canonical key names and default sizes in [`docs/reference/config.md`](../reference/config.md)). The full stderr is in the per-visit capture file (Tier 2).
...
- **A `--debug` CLI flag and a per-issue debug log directory** are out of scope; per-visit captures cover the same need and are always on.
...
- **Related reference**: [`docs/reference/log-events.md`](../reference/log-events.md), [`docs/reference/config.md`](../reference/config.md) (`[log]` section).

...
- **Reserved extension namespaces**: regions where each downstream spec places its own keys.
  ...
  - `extension.phase.<name>.*` — per-phase override surface; the canonical key under each phase is `command`, the slash-command override that swaps the catalog default skill (per [FR 18 §Phase override](18-worker-skill-workflow.md)). Mutually exclusive per phase with `prompt_template_<phase>`. The `max_turns` key is also exposed per phase.

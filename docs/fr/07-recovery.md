...
1a. Pre-seed the in-memory diff cache from the SQLite control-plane store's last-admitted set (best-effort; store errors are non-fatal). The GraphQL enumerate in step 2 overwrites or evicts each seeded entry as Linear truth dictates.
1b. Close out crash-interrupted cycles: every `cycles` row with `ended_at IS NULL` is stamped `outcome = failure` with the current timestamp, and the daemon emits one `crashed_cycle_recovered` event per row (carrying `ticket_id`, `cycle_id`, `kind`, `last_state`). Best-effort; store errors are warn-logged and recovery continues.

...
> Per-ticket session tempdirs (always present once admitted) and per-ticket git worktrees (lazily materialized when a state first emits `directive: "run"`). The daemon owns both directories' creation and deletion. Operators read paths through `roki repo` ([09-log-access-cli](09-log-access-cli.md)); state subprocesses walk into the resolved path.
...
- **Created lazily**: when a state's sentinel directive resolves to `directive: "run"` and the worktree does not yet exist, the daemon creates it before spawning the next state's subprocess. The repo is the admission-resolved repo for this ticket ([03-linear-admission §Repo resolution](03-linear-admission.md)).

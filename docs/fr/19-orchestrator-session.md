...
| `stall_seconds` | `120` | Orchestrator-stall window: if the orchestrator emits no stdout for this many seconds, the daemon SIGTERMs the orchestrator and routes the issue to `Inactive(reason=orchestrator_crash)` per `roki-mvp Req 5.3` / §Failure modes |
...
| Orchestrator stall (no stdout emitted for `extension.orchestrator.stall_seconds`, default `120`) | daemon SIGTERMs the orchestrator → routes through the crash path | `orchestrator_crash` |

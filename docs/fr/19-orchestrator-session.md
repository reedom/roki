...
The orchestrator does not edit code (no Edit, no Write) but can run shell and read files (Bash + Read inside a read-only filesystem sandbox) for `stat` / `test -f` / `grep` checks against artifacts and project-level spec docs. The daemon never writes Linear directly and never decides artifact pass/fail — both are the orchestrator's job.

Rejected alternatives: putting Linear-label parsing inside the orchestrator (wastes a thinking turn on a mechanical decision); accepting `roki:impl` alone for SPEC_DRIVEN admission (collapses operator's two-step authorize + declare-spec-ready intent into one label slip); driving SPEC_DRIVEN through `materialize_spec` + `kiro-spec-quick --auto` (forces spec materialization even when the operator already has one); auto-detecting "spec complete enough" via `spec.json` approvals heuristics (turns operator intent into a guess with no escape hatch).
...
| `tracker_terminal` | Linear state moved to `done` / `canceled`, assignment was lost, or `roki:ready` was removed ([04-state-machine-and-recovery](04-state-machine-and-recovery.md)). Preempts an in-flight phase: the daemon SIGTERMs the phase subprocess, awaits its exit, **discards** the resulting terminal event without translating it into `phase_complete` / `phase_nonclean` (captures it in the structured log only), and delivers `tracker_terminal` solo. Translating the SIGTERM-induced exit would burn an orchestrator turn on a decision the next event already overrides. | Return `action=stop` with `outcome=cancelled`; cleanup follows. |
...
The daemon does not deliver events to the orchestrator while a phase subprocess is running — it waits for the phase to terminate, observes the exit, then sends the matching `phase_complete` / `phase_nonclean`. The single exception is `tracker_terminal`: when a tracker terminal observation arrives mid-phase, the daemon SIGTERMs the in-flight phase, awaits its exit, **discards** the resulting terminal event without translating it into `phase_complete` / `phase_nonclean`, and delivers `tracker_terminal` solo. The discarded exit is captured in the structured log only.
...
| `stall_seconds` | `600` | Orchestrator-stall window: if the orchestrator emits no stdout for this many seconds, the daemon SIGTERMs the orchestrator and routes the issue to `Inactive(reason=orchestrator_crash)` per `roki-mvp Req 5.3` / §Failure modes. Default is sized for `effort=high` turns that combine extended-thinking blocks with tool calls; operators on `effort=middle`/`low` may lower the value |
...
| Orchestrator stall (no stdout emitted for `extension.orchestrator.stall_seconds`, default `600`) | daemon SIGTERMs the orchestrator → routes through the crash path | `orchestrator_crash` |
...
- **Bounded thinking budget**: `extension.orchestrator.{model, effort, max_phases, allowed_tools, stall_seconds}` lets the operator cap the orchestrator's thinking effort (`effort`), total phase nominations (`max_phases`), and stall-detection window (`stall_seconds`) without per-process `--max-turns`.

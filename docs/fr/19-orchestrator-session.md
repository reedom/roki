...
> The long-lived per-ticket `claude --input-format stream-json --output-format stream-json` "thinking" component (A) that absorbs admission decisions, phase planning, daemon-directive interpretation, and Linear writes — driving zero or more short-lived phase subprocesses for the actual code-changing work.

## Purpose

The previous architecture launched a single bounded `claude --print` worker per admitted issue plus two auxiliary one-shot subprocesses (a setup judge for admission classification and a linear-updater subagent for daemon-only failure surfacing). That model gave the daemon no structured handle for per-phase budgets, no clean way to cap thinking effort, and forced two extra prompt templates and lifecycles to surface admission and Linear writes.

This FR canonicalizes the replacement: a long-lived **orchestrator session** (A), launched per ticket as `claude --input-format stream-json --output-format stream-json`, that absorbs both auxiliary roles and emits phase requests as strict JSON action directives. The actual code-changing work runs in short-lived phase subprocesses ([18-worker-skill-workflow](18-worker-skill-workflow.md)) that A nominates via `run_phase`. A is the only "thinking" component the daemon launches per ticket; it is where admission classifies, phases get planned, daemon-only failure directives get translated into Linear writes, and the ticket's terminal `stop` action gets emitted. A does not edit code, run shell, or dispatch agents. The daemon itself never writes Linear directly.

Rejected alternatives: keeping the prior worker shape and adding a separate per-phase planner (would have created a third "thinking" surface with no clear seam to the existing two); driving phase choice from the daemon ("what's next") in Rust (conflicts with the skill-first pivot — the daemon is mechanical observation only).
...
| `phase_complete` | a phase subprocess clean-exited with `subtype: success` | return `action=run_phase` (next phase) or `action=stop` (`outcome=success` or `outcome=failure`) |
...
| `daemon_directive` | a daemon-only failure that A is alive to surface (stall on a sibling phase, retry exhaustion, filesystem poison, judge contradiction, etc.) | A writes the appropriate Linear label + comment via Linear MCP and returns `action=linear_update_done` |
| `gate_deny` | the review gate returned `Deny+RetryWithContext(payload)` ([09-pre-pr-gate](09-pre-pr-gate.md)) | return `action=run_phase` with `phase=implement` and populate `additional_context` from the payload |
...
- **Linear MCP (write)** — the operator's installed Linear MCP, used by A for label + comment writes (admission rejections and daemon-directive surfacing).
- **Read** (workspace, read-only) — A may read ticket-related files inside the worktree if useful for its turn.
- **No** Bash, **no** Edit, **no** Write, **no** Agent dispatch, **no** other MCPs.
...
| `allowed_tools` | Linear MCP (write) + `Read` | Allowlist passed via `--settings` |
...
- **Single thinking component per ticket**: A absorbs admission classification, phase planning, daemon-directive surfacing, and Linear writes into one long-lived session. The previous setup-judge and linear-updater subprocess shapes are removed.
...
- **Read + Linear-MCP-write only**: enforced by `--settings`; A cannot edit code, run shell, or dispatch agents. Code-changing work is the phase subprocess's role.
- **Single template block**: `prompt_template_orchestrator` replaces the prior three blocks; one prompt covers admission, phase planning, and daemon-directive surfacing.
...
- **A does not edit code, run shell, invoke `gh`, or push to git** — that is the phase subprocesses' role ([18-worker-skill-workflow](18-worker-skill-workflow.md)).
...
- **Related FR**: [01-daemon-lifecycle](01-daemon-lifecycle.md), [04-state-machine-and-recovery](04-state-machine-and-recovery.md), [07-worker-execution](07-worker-execution.md), [09-pre-pr-gate](09-pre-pr-gate.md), [11-agent-tool-boundary](11-agent-tool-boundary.md), [12-extension-surface](12-extension-surface.md), [14-operator-notifications](14-operator-notifications.md), [18-worker-skill-workflow](18-worker-skill-workflow.md).

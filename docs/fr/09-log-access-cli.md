...
> Three small CLIs (`roki log`, `roki events`, `roki repo`) that operators (and operator-authored states) call to inspect per-ticket subprocess captures, the structured event stream, and per-ticket repo paths. The CLIs encapsulate the daemon's storage layout so the on-disk format can change without breaking workflow scripts.

## Purpose

The cycle engine ([01-engine-model](01-engine-model.md)) writes three kinds of artifact: per-visit subprocess captures, structured event log entries, and per-ticket worktree state. Workflow templates and operator tooling read all three. Exposing the on-disk layout directly would freeze the storage backend; exposing only Linux paths in environment variables also makes the API platform-specific. These CLIs sit between operator code and the storage layer, so the daemon can switch from flat files to SQLite or a remote store later without breaking workflow templates.

## User-visible Behavior

### `roki log` — per-ticket subprocess capture

Reads stdout, stderr, exit code, sentinel directive payload, and parsed terminal event for a given (ticket, cycle, visit, state). Default arguments come from the environment variables the daemon injects when spawning a state, so most invocations are short.

```bash
# Defaults: --ticket $ROKI_TICKET_ID --cycle $ROKI_CYCLE_ID --iter <latest completed>
roki log --state impl --stream stdout

# Relative iter (negative = N visits back from current; cycle-wide visit ordering)
roki log --iter -1 --state verdict --stream directive

# Range / tail
roki log --iter -1 --state impl --stream stdout --tail 50      # last 50 lines
roki log --iter -1 --state impl --stream stderr --bytes 4096   # last 4 KiB
...
`--state <state_id>` selects which state's files within the visit. `--iter <n>` selects the visit; absolute (`1`, `2`, ...) or relative (`-1` = previous, `-2` = two back). Visit numbering is cycle-wide: every state spawn increments the cycle's `visit_n` regardless of which state it lands in.

Scope: a `roki log` invocation is bound to a single ticket. The daemon refuses cross-ticket reads (an operator-authored failure handler for ticket A cannot accidentally read ticket B's logs). The `--ticket` flag exists for TUI / external tooling, not for cross-ticket reads from inside a state: when invoked without environment, the operator must supply both `--ticket` and `--cycle`.
...
When the requested repo matches the admission-resolved repo for the current ticket and the daemon has materialized the worktree, `roki repo` returns the worktree path. Otherwise it returns the ghq base path so pre-worktree inspection still has somewhere to read from. `--auto-clone` enables `ghq get` for the ghq base; the daemon never auto-clones implicitly.

Pre-worktree callers receive the ghq base path — the operator's main checkout — and any write operation there (`git commit`, `git checkout`, etc.) pollutes the shared clone. Treat the default `roki repo` result as **read-only** unless `--worktree` confirms a per-ticket worktree has been materialized. States that need to write should either run after a state that materializes the worktree, or pass `--worktree` and handle the exit-1 case explicitly.
...
This layout is **not** part of the operator-facing contract. Future versions may move some files into a SQLite database, compress old visits, or delegate to a remote store; only the CLIs are stable.

## Capabilities

- **One-line accessors for the common case**: env-var defaults make `roki log --state <id> --stream stdout` a useful shorthand inside any workflow template.
- **Relative visit arithmetic**: `--iter -N` for "N visits back" lets a downstream state inspect the previous one trivially.
- **Same-ticket isolation**: state subprocesses cannot accidentally read a different ticket's captures.
...
- **Cross-ticket reads from inside a state** are not supported. The TUI and external scripts may pass `--ticket` explicitly, but a workflow template launched by the daemon for ticket A cannot read ticket B's captures via these CLIs.

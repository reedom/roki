...
The webhook receiver is mandatory (`[linear.webhook]` is required in `roki.toml`). Polling is the daemon-side Linear pull path. It activates from two triggers:

- **Webhook outage detection** — Linear cloud unreachable, network partition, the webhook receiver port becoming temporarily unbindable. The daemon polls on cadence until webhook delivery resumes.
- **Refresh nudge** — `POST /api/refresh` from operator tooling (TUI, scripts, observability components); a one-shot poll outside the outage path (see [§Refresh nudge](#refresh-nudge)).

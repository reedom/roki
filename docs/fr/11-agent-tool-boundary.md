...
- **Linear writes**: every Linear write originates from an agent invocation through the operator-configured Linear MCP. Specifically:
  - **A** writes Linear via Linear MCP for operator-facing pre-phase stops (`outcome ∈ {needs_split, allowlist_rejected, spec_incomplete, needs_operator}`) and for daemon-only failure directives (`daemon_directive` events — see [FR 14: Operator Notifications](14-operator-notifications.md)).

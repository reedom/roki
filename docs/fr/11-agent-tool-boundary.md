...
> The principle that the daemon **never registers, proxies, or wraps any agent-side tool**. Every agent invocation (worker, setup judge, linear-updater) inherits the operator's local Claude Code tool surface verbatim. Each downstream gate spec may register exactly one **read-only self-diagnosis tool** for the agent's benefit.
...
The same principle applies to all three agent invocation roles: the main worker, the setup judge, and the linear-updater subagent. None of them is a special daemon-internal route — they are all `claude` subprocesses inheriting the operator's MCP surface.
...
- **Subprocess tool surface**: every agent subprocess (worker / judge / linear-updater) sees the built-in tool set exposed by the operator's Claude Code installation plus the MCP servers the operator has installed, as-is. The daemon adds nothing, removes nothing, and wraps nothing.
- **Linear writes**: every Linear write originates from an agent invocation through the operator-configured Linear MCP — the worker's kiro skill writes status updates / PR linkage / fix-finding context, and the linear-updater subagent writes daemon-only failure labels and comments. The daemon process never issues a Linear write.
...
These tools are typically used by the worker's kiro skill to read context like "what's the artifact path for this issue's `requirements.md`?" or "what was the last review-gate failure reason on the previous turn?" — they do not gate the worker's own loop.
...
- **Same boundary across all three roles**: worker, setup judge, and linear-updater all inherit the operator's tool surface. Linear writes from the linear-updater go through the operator's Linear MCP, not through any daemon-side write path.
...
- **Related FR**: 07-worker-execution, 08-pre-implementation-gate, 09-pre-pr-gate, 14-operator-notifications

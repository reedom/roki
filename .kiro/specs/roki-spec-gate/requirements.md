...
**Objective:** As an operator, I want the spec gate to subscribe to roki-mvp's vetoable `Judging -> Active` transition hook, so that no worker can begin implementation work for a `(repo, issue)` until the gate has explicitly allowed it.

#### Acceptance Criteria
1. When the roki daemon starts and the spec-gate component is enabled, the spec gate shall register a subscriber on roki-mvp's state-machine subscription hook for the vetoable `Judging -> Active` transition before any worker is promoted to `Active`.
2. When roki-mvp publishes a vetoable `Judging -> Active` transition event for a `(repo, issue)`, the spec gate shall return an allow decision only after its own gate evaluation produces a pass outcome for that `(repo, issue)`.
3. While the spec gate is evaluating a `Judging -> Active` transition for a `(repo, issue)`, the spec gate shall not process a second concurrent evaluation for the same `(repo, issue)`.
...
5. The spec gate shall not subscribe to or veto any transition other than `Judging -> Active`.
...
#### Acceptance Criteria
1. When the spec gate begins evaluating a `Judging -> Active` transition for a `(repo, issue)`, the spec gate shall invoke a spec-materialization turn against the agent session for that `(repo, issue)` whose only stated purpose is to produce `.kiro/specs/<issue>/requirements.md` by merging the Linear ticket body with the project's existing EARS docs.
...
5. If the configured attempt cap is exhausted without a passing attempt, the spec gate shall return a deny decision to the orchestrator for the `Judging -> Active` transition and shall emit an escalation event for that `(repo, issue)`.
...
#### Acceptance Criteria
1. When the spec gate's evaluation produces a passing validation result for a `(repo, issue)`, the spec gate shall return an allow decision to the orchestrator for the `Judging -> Active` transition.
...
**Objective:** As an operator running roki across multiple repositories, I want the gate to work correctly when several `(repo, issue)` pairs progress through `Judging -> Active` concurrently, so that one slow or failing gate evaluation cannot starve or contaminate evaluations for other tickets.
...
2. While the orchestrator may publish duplicate `Judging -> Active` transition events for the same `(repo, issue)` (for example, due to webhook redelivery), the spec gate shall return the same decision for the same logical attempt without double-counting against `extension.gates.spec.max_attempts`.

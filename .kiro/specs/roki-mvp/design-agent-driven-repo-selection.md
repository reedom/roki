...
| 7 | Linear admission filter | A. Admit every issue → agent decides whether to do work, exits early if not<br>B. Deterministic pre-filter (e.g., `[admission].required_label = "roki"`)<br>C. Linear-state pre-filter (e.g., admit only `Todo`) | **A** for MVP; revisit if cost is real. WORKFLOW.md gates handle "is this work actually mine?" with a cheap first-turn agent decision. |
| 8 | What happens when agent never calls `roki_open_worktree` | A. CleanExit advances to `AwaitingReview` regardless<br>B. Daemon enforces "at least one worktree opened" before allowing AwaitingReview | **A** — agent might legitimately decide "this isn't for me" and exit. Validate via `MONORAIL_RESULT`-style structured exit signal in WORKFLOW.md gate (out of scope for 7.1 itself). |
...
- `[linear]` config block with `token_env` (replaces the implicit `LINEAR_API_TOKEN` default) and `webhook_secret_env`.
...
State machine itself doesn't change — it's still `Discovered → Queued → Active → AwaitingReview → TerminalSuccess → Cleaning` with `Backoff`/`TerminalFailure` branches. What changes is the work done at each transition:
...
7. Forward to orchestrator's tracker_inbox keyed by IssueId
8. Return 204
```

No per-repo dispatch. The orchestrator spawns a worker for any new IssueId it sees that isn't already in flight.
...
3. For every distinct issue id discovered (from either source), query Linear for the current state.
4. Reconcile per the existing four-cell matrix (`ResumeActive` / `OrphanedSession` / `FreshQueued` / `NoOp`). Note: `OrphanedWorkspace` becomes `OrphanedSession`-or-`OrphanedWorktree` depending on what survived.
...
Production source:
- `crates/roki-daemon/src/config/mod.rs` — drop `RepoConfig.id` etc.; add `[linear]` and `[workflow]` blocks.
...
- `crates/roki-daemon/src/tracker/{linear.rs,webhook.rs}` — single `LinearTracker` (no scope), single webhook route.
...
- Linear admission pre-filter (e.g., `[admission].required_state`). Punt; revisit only if "every Linear update spawns a worker" becomes a real cost problem.
...
- **#7 (admit-everything filter)** — daemon spawns a worker for every Linear update unless WORKFLOW.md has a gate that exits cheaply. Could be wasteful at high update volumes. Acceptable?

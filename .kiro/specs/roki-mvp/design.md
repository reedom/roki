...
| OrchestratorSessionAdapter | Engine | Launch and supervise the long-lived `claude --input-format stream-json --output-format stream-json` orchestrator session; render `prompt_template_orchestrator` with mode flag substituted in; deliver event JSON to its stdin; parse last JSON object per turn from stdout; validate against the `OrchestratorAction` schema; reprompt once on schema drift; enforce `max_phases`; detect orchestrator-stall via the `extension.orchestrator.stall_seconds` window (default `120`) | 4.1, 4.2, 4.7, 5.1, 5.2, 5.3, 5.4, 5.5, 6.6, 7.1, 9.6, 11.1, 11.5, 11.8 | tokio process (P0), Permissions (P0), WorkflowLoader (P0), ActionParser (P0) | Service, Event |
...
- Detect orchestrator-stall via `extension.orchestrator.stall_seconds` (default `120`; no stdout for that many seconds). On exceed: SIGTERM the orchestrator, route to `Inactive(orchestrator_crash)`, no Linear write.
...
pub struct OrchestratorLaunchContext {
    ...
    pub stall_seconds: u32,                  // extension.orchestrator.stall_seconds; default 120
}
...
- Drive the ticket-level `max_attempts` retry-budget loop on `phase_nonclean` (default 3, range 1..10). On exhaustion: deliver `daemon_directive(retry_exhausted)` to the orchestrator session if alive; the orchestrator emits `action=stop outcome=failure`. While remaining attempts exist, transition `Active → Backoff`, apply exponential backoff bounded between 10s and 5min, retain the session tempdir and worktree, and on timer expiry transition back to `Active` for the orchestrator's next phase nomination.

...
- [x] 10.8 Enqueue EscalationEntry on orchestrator ProcessExit
...
- **Mid-phase abort SIGTERM gap — RESOLVED by 10.7:** `engine/claude.rs::ClaudeSpawn::spawn` now sets `kill_on_drop(true)` on the `Command` builder, so dropped `tokio::process::Child` handles SIGKILL the spawned subprocess. Closes Req 1.4 mid-phase abort guarantee for both orchestrator session + phase Children.

- **ProcessExit escalation enqueue gap — RESOLVED by 10.8:** `crates/roki-daemon/src/orchestrator/core.rs` now extracts a `route_orchestrator_dead` helper applied to all three orchestrator-dead paths in `drain_actions` (channel-closed, ProcessExit, TerminalDrift). Enqueues `EscalationEntry` BEFORE `transition_to_inactive`, mirroring `handle_daemon_escalation`. Closes Req 12.3 queue-population half. The 13.6 + 13.7 seam tests' belt-and-braces `DaemonEscalation` send is now redundant in production but harmless (latest-wins enqueue by issue+kind).

...
refs:
  ...
  provides:
    ...
    - req:roki-observability:15.3a
---

# Requirements Document

## Project Description (Input)
roki-observability adds an optional axum HTTP server module to the roki daemon and a separate `roki-tui` ratatui binary so operators can see active workers, per-issue state, the last lifecycle event, the escalation queue, and token / rate-limit snapshots. The HTTP server is loopback-only by default, gated by `WORKFLOW.md` `server.port` (off by default), and exposes a versioned JSON API: `GET /api/v1/state`, `GET /api/v1/<issue>`, `POST /api/v1/refresh`. The TUI is a separate process that reads the JSON API on a refresh loop and supports escalation acknowledgement plus a manual refresh nudge. Read-only projection in v1: no cancel, retry, or reschedule; `POST /refresh` only nudges the poller. All agent-derived strings are HTML-escaped and ANSI-stripped before rendering. Shared `serde` types live in a single crate consumed by both sides so the API schema cannot drift. Out of scope: authn / auth tokens, web UI, mutating control plane beyond `/refresh`, persistent metrics. Reuse roki-mvp's in-memory state model — do not duplicate state structs.

## Introduction

The HTTP module is a read-only projection of roki-mvp's in-memory state: it subscribes to the orchestrator's transition event bus and serves snapshots; the orchestrator does not depend on the API and the daemon continues to function with the API disabled. The JSON schema is versioned under `/api/v1/` and centralized in a single shared crate so server, TUI, and future external dashboards consume one stable contract.

This spec extends roki-mvp by plugging into its declared extension seams: the transition event bus (Requirement 8.2 and 8.3 of roki-mvp) and the `WORKFLOW.md` schema (Requirement 6.5 of roki-mvp). It does not modify the orchestrator state machine and does not add mutating endpoints beyond a refresh nudge.

## Boundary Context

- **In scope**: an optional axum HTTP server module compiled into the roki daemon binary and gated by a `WORKFLOW.md` `server.port` key (off by default); three JSON HTTP endpoints `GET /api/v1/state`, `GET /api/v1/<issue>`, `POST /api/v1/refresh`; versioned JSON schema under `/api/v1/` defined in a single shared crate; loopback-only default bind with operator opt-in to bind on a non-loopback interface; HTML-escaping plus ANSI stripping of every agent-derived string field before serialization; a separate `roki-tui` ratatui binary that consumes the JSON API on a refresh loop and renders the active worker list, per-issue state and last event, escalation queue, token / rate-limit snapshots; TUI escalation acknowledgement (a local UI flag, no daemon-side effect in v1) and a TUI manual refresh action that issues `POST /api/v1/refresh`; macOS plus Linux terminal compatibility with iTerm2 / Ghostty / WezTerm / Alacritty as the primary targets and a graceful informational degradation on macOS Terminal.app for RGB color limits; a single shared crate of `serde` types consumed by both the server module and the TUI client so the schema cannot drift; configuration of refresh cadence and bind address through `WORKFLOW.md` and CLI flags; structured logging of every API request (method, path, status, duration, correlation id) through the existing tracing pipeline.
...
2. The roki daemon shall serialize each per-issue entry in the snapshot with the issue identifier (one ticket maps to at most one repo per `roki-mvp` design.md `Multi-repo tickets (rejected by the orchestrator with outcome=needs_split)`; the resolved single repo is included as an informational field, not a compound key), current `WorkerState`, the latest `Inactive(reason=...)` discriminator value when the worker is in the `Inactive` state, the most recent lifecycle event summary, the timestamp of the last lifecycle event, and the current correlation identifier.
3. The roki daemon shall produce the snapshot by reading the orchestrator state map, the per-issue event cache, and the escalation queue in that fixed order; readers may observe sub-snapshots taken up to the snapshot drift bound apart, in which case an entry's `state`, its most recent lifecycle event, and its `EscalationEntry` may reflect adjacent moments rather than one instant. The snapshot drift bound shall be no greater than 50 milliseconds under nominal load on a developer-class machine, shall be documented in `SPEC.md`, and shall be asserted by an integration test that drives concurrent transitions during snapshot assembly.
...
**Objective:** As an API consumer, I want a per-issue endpoint that returns deeper detail for a specific issue than the global snapshot includes, so that I can drill into one ticket without scanning the whole state response.

#### Acceptance Criteria
1. When a client sends `GET /api/v1/<issue>`, the roki daemon shall return HTTP 200 with a JSON body containing the per-issue detail (issue identifier, resolved repo as an informational field, current state, latest `Inactive(reason=...)` discriminator value when applicable, recent lifecycle event log up to the documented retention window, last error if any, configured permission strategy, workspace path). One ticket maps to at most one repo per `roki-mvp` design.md (multi-repo tickets are rejected by the orchestrator with `outcome=needs_split`); the issue identifier alone uniquely identifies the entry.
2. The roki daemon shall accept the `<issue>` path segment as the unique issue identifier; no `repo` query parameter is supported (the API rejects it as an unknown query parameter at startup-asserted route configuration time, not runtime). Any future re-introduction of multi-repo tickets requires a follow-up spec amendment.
3. If the requested issue is not present in the in-memory state, the roki daemon shall return HTTP 404 with a JSON error body that names the issue identifier.
...
### Requirement 5: Stable Versioned JSON Schema in a Single Shared Crate

**Objective:** As a maintainer of the daemon, the TUI, or a future external dashboard, I want the JSON contract to be defined in one shared crate and versioned under a stable URL prefix so that server, client, and any future consumer cannot drift on field names and breaking changes are explicit rather than silent.

#### Acceptance Criteria
1. The roki daemon shall serialize every response from `GET /api/v1/state`, `GET /api/v1/<issue>`, and `POST /api/v1/refresh` using `serde` types defined in the shared `roki-api-types` crate; the daemon shall not re-declare any of these types locally.
2. The roki-tui binary shall deserialize every response from those endpoints using the same `serde` types from the shared `roki-api-types` crate; the TUI shall not re-declare any of these types locally.
3. The shared `roki-api-types` crate shall expose a `pub const API_VERSION: &str = "v1";` and every JSON response body shall include an `api_version` field whose value matches that constant.
4. The roki daemon shall document every field of every response shape, including roki-specific fields such as multi-repo `(repo, issue)` keying and `inactive_reason`, in `SPEC.md` under the `## Observability HTTP API` section.
5. The roki daemon shall version the JSON contract under the `/api/v1/` prefix so that future breaking changes introduce a `/api/v2/` namespace without disturbing existing `/api/v1/` consumers; additions to existing v1 shapes shall be additive only (new optional fields), never renames or removals.
...
2. When the operator invokes `roki-tui` with a target API URL, the roki-tui binary shall connect to that URL, fetch the initial snapshot via `GET /api/v1/state`, and render the live state view within the startup window of one second on a developer-class machine against a loopback daemon, where the startup window shall be documented in `SPEC.md`.
...
4. The shared crate shall be the only place where the issue-keyed projection (issue identifier + informational repo), `WorkerState` projection, lifecycle event summaries, escalation entries, and token / rate-limit snapshot shapes are declared for the API.
...
3a. If a `server.*` value fails type or range validation during a hot reload of `WORKFLOW.md`, the roki daemon shall keep the previously loaded policy in effect, shall log a structured error that names the offending key, and shall not crash the daemon (consistent with `fr:02-configuration` hot-reload semantics).

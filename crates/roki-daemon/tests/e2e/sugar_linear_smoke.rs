//! Slice 8 e2e: three-task `tasks:` sugar expands into a state chain;
//! all three states run in declared order, last `on_done` falls through to
//! the auto-injected `__success__` terminal. Spec §12.1 "slice8-sugar-linear".
...
#[tokio::test]
async fn sugar_three_task_chain_terminates_success() {
    ...
    let ticket_id = "ENG-810";
    let order_path = work.path().join("order.txt");
    let order_str = order_path.display().to_string();
...
"#,
    ...
    let order = std::fs::read_to_string(&order_path).expect("order file written");
    assert_eq!(
        order, "abc",
        "states must run in declared order, got {order:?}"
    );

    let body = std::fs::read_to_string(&events_path).unwrap();
    let last = body.lines().last().expect("at least one event");
    assert!(
        last.contains("\"event\":\"cycle_completed\""),
        "last line must be cycle_completed: {last}"
    );
    assert!(

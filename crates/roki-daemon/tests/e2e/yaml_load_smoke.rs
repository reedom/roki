//! Slice 8 e2e: minimal `WORKFLOW.yaml` parses + validates, the daemon
//! emits `daemon_ready` to `_daemon.events.jsonl`, no webhook traffic
//! required. Covers spec §12.1 "slice8-yaml-load" acceptance.
...
#[tokio::test]
async fn minimal_yaml_loads_and_daemon_ready_fires() {

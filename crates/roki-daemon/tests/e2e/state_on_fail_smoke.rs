//! Slice 8 e2e: state exits non-zero, the state's `on_fail` edge is taken.
//! No daemon-detected failure is emitted, no `on_failure:` rule runs, the
//! cycle stays `kind: rule` and terminates at the auto-injected
//! `__success__` after the recovery state. Spec §12.1 "slice8-state-on-fail".
...
#[tokio::test]
async fn nonzero_exit_takes_state_on_fail_edge() {
...
rules:
  - when:
    ...
    states:
      a:
        run: 'printf a >> "{trace}"; exit 1'
        on_fail: recover
      recover:
        run: 'printf r >> "{trace}"; exit 0'
        on_done: __success__
    terminals: {{}}
"#,
        trace = trace_str
...
"#,
    ...
    let trace = std::fs::read_to_string(&trace_path).expect("trace");
    assert_eq!(
        trace, "ar",
        "on_fail edge must visit recover after a, got {trace:?}"
    );

    let body = std::fs::read_to_string(&events_path).unwrap();
    assert!(
        !body.contains("\"cycle_kind\":\"failure\""),
        "no failure cycle expected; state on_fail must stay in rule cycle: {body}"
    );
    assert!(
        !body.contains("\"event\":\"failure_unhandled\""),
        "no failure_unhandled expected: {body}"
    );

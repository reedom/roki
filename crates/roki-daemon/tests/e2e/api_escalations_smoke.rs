//! Slice 9 e2e: drive a workflow whose recursion-bounded rule emits a
//! `recursion_bound` failure that also recursion-bounds on its
//! on_failure handler → an `escalation_added` lands. Then
//! `GET /api/escalations` returns the queued entry. Spec fr:10
//! §`GET /api/escalations`.
...
#[tokio::test]
async fn escalations_endpoint_returns_queued_recursion_bound() {
    ...
    let ticket_id = "ENG-904";

    // Same recursion-bound pattern as recursion_bound_yaml_smoke: rule's
    // `a` self-loops with max_visits=2, on_failure handler's `h` also
    // self-loops to recursion_bound. The terminal failure-cycle failure
    // routes to the escalation queue.
...
"#,
    ...
    let escs: serde_json::Value = client
        .get(format!("http://127.0.0.1:{api_port}/api/escalations"))
        .send()
        .await
        .unwrap()
        .json()
        .await
        .unwrap();
    let arr = escs.as_array().expect("escalations body must be array");
    assert_eq!(arr.len(), 1, "expected one escalation, got: {escs}");
    assert_eq!(arr[0]["kind"], "recursion_bound");

//! Slice 9 e2e: admit one ticket via webhook, await a cycle, then
//! `GET /api/tickets` should return a single-entry array with the
//! admitted ticket id and `status = "in_progress"`. Spec fr:10
//! §`GET /api/tickets`.
...
#[tokio::test]
async fn tickets_endpoint_lists_admitted_ticket() {
...
"#,
    ...
    let api_resp = client
        .get(format!("http://127.0.0.1:{api_port}/api/tickets"))
        .send()
        .await
        .expect("tickets GET");
    assert_eq!(api_resp.status().as_u16(), 200);
    let body: serde_json::Value = api_resp.json().await.expect("tickets json");
    let arr = body.as_array().expect("tickets body must be array");
    assert_eq!(arr.len(), 1, "exactly one admitted ticket, got: {body}");
    assert_eq!(arr[0]["ticket_id"], ticket_id);

//! Slice 9 e2e: `[api].port` absent → daemon emits `api_disabled`,
//! continues to `daemon_ready`, and exits 0 on SIGTERM. Spec fr:10
//! "server gating".
...
#[tokio::test]
async fn api_disabled_when_port_absent() {
...
"#,
    ...
    // `api_disabled` is emitted right after `daemon_ready` in the
    // current runtime ordering; await both with generous budgets.

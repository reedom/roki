//! Slice 8 e2e: canonical state-machine form (`start:`/`states:`/`terminals:`).
//! State `a` writes a sentinel `{"directive":"skip"}`; the state declares
//! `directives: { skip: __no_action__ }`, so the cycle terminates at the
//! auto-injected `__no_action__` terminal with `outcome: no_action`.
//! Spec §12.1 "slice8-canonical-branch".
...
#[tokio::test]
async fn canonical_skip_directive_terminates_no_action() {
...
rules:
  - when:
    ...
    states:
      a:
        run: 'printf ''{"directive":"skip"}'' > "$ROKI_DIRECTIVE_PATH"'
        directives:
          skip: __no_action__
    terminals: {}
"#;
...
"#,
    ...
    assert!(
        last.contains("\"terminal_id\":\"__no_action__\""),
        "must terminate at __no_action__: {last}"
    );
    assert!(
        last.contains("\"outcome\":\"no_action\""),
        "outcome must be no_action: {last}"
    );
}

//! Slice 9 e2e: drive one cycle for an admitted ticket, then exercise
//! `GET /api/tickets/{id}/cycles` and
//! `GET /api/tickets/{id}/cycles/{cycle_id}/visits/{n}/{state_id}/stdout`.
//! Spec fr:10 §`GET /api/tickets/{id}/cycles` + §visit stream.
...
#[tokio::test]
async fn cycles_and_visit_stdout_match_executed_state() {
    ...
    let ticket_id = "ENG-902";

    let workflow_path = work.path().join("WORKFLOW.yaml");
    // Single sugar-form task `solo` that prints `out` to stdout. The state's
    // capture file (`solo.stdout`) is then served via the visit endpoint.
    let workflow_body = r#"
admission:
  assignee: u1
  repos:
...
"#,
    ...
    // /api/tickets/{id}/cycles returns { cycles: [...], truncated }.
    let cycles: serde_json::Value = client
        ...
        .await
        .unwrap();
    let cycle_id = cycles["cycles"][0]["cycle_id"]
        .as_str()
    ...
    let stdout = client
        .get(format!(
            "http://127.0.0.1:{api_port}/api/tickets/{ticket_id}/cycles/{cycle_id}/visits/1/{state_id}/stdout"
        ))
        .send()
        .await
        .unwrap()
    ...
    assert!(
        stdout.contains("out"),
        "visit stdout must contain 'out', got {stdout:?}"
    );

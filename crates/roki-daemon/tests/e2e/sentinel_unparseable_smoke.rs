//! Slice 8 e2e: state writes invalid JSON to `$ROKI_DIRECTIVE_PATH`, exits 0.
//! Daemon classifies the result as `unparseable`; the matching `on_failure:`
//! rule (`when.kind: unparseable`) runs as a `kind: failure` cycle. The
//! handler writes `{"directive":"end"}`, so the daemon stays alive and emits
//! exactly one `cycle_completed` with `cycle_kind: failure`.
//! Spec §12.1 "slice8-sentinel-unparseable".
...
#[tokio::test]
async fn unparseable_sentinel_routes_to_on_failure_handler() {
...
rules:
  - when:
    ...
    tasks:
      - id: bad
        run: 'printf not-json > "$ROKI_DIRECTIVE_PATH"; exit 0'
...
on_failure:
  - when:
    ...
    tasks:
      - id: handler
        run: 'printf ''{"directive":"end","outcome":"recovered"}'' > "$ROKI_DIRECTIVE_PATH"'
"#;
...
"#,
    ...
    assert!(
        ...
        "handler must run as failure cycle: {body}"
    );
    assert!(
        !body.contains("\"event\":\"failure_unhandled\""),
        "handler must consume the failure: {body}"
    );
}
